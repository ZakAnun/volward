  ...
  group('mergeSubtreeIntoSnapshot', () {
    ...
    test('replaces only the targeted subtree, leaving siblings untouched', () {
      ...
      expect(
        downloads['scanned'],
        isFalse,
        reason: 'sibling must be untouched',
      );
    ...
    test(
      ...
      expect(children.any((c) => c['path'] == '/root/Documents'), isTrue);
    ...
    test(
      ...
      expect(documents['size_bytes'], 500);
      expect(
        kids.any((c) => c['path'] == '/root/Documents/peeked.txt'),
        isTrue,
      );
    ...
    test(
      'adopts a newly-discovered child the checkpoint brings that was not known before',
      () {
        final merged = mergeSubtreeIntoSnapshot(
          snapshot: baseSnapshot(),
          targetPath: '/root',
        ...
        expect(
          children,
          hasLength(3),
          reason: 'Documents + Downloads + new Movies',
        );
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          isTrue,
          reason:
              'SubFolder (level-3) must be stamped scanned:true recursively',
        );
    ...
    test(
      'adopts checkpoint data once it progresses past the existing state',
      () {
        final merged = mergeSubtreeIntoSnapshot(
          snapshot: baseSnapshot(),
          targetPath: '/root',
    ...
    test(
      ...
      expect(
        documents['size_bytes'],
        3000,
        reason: 'trusts the fresh, smaller size',
      );
