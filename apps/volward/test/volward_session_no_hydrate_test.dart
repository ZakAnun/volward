...
void main() {
  ...
  group('session state file resilience', () {
    test(
      'concurrent session loads share one read and restore the root',
      () async {
        final session = VolwardSession.test();
        final file = File(
    ...
    test(
      'does not restore an old root after the user switches folders',
      () async {
        final session = VolwardSession.test();
        final file = File(
    ...
    test(
      'loadSessionStateIfNeeded ignores empty and malformed session files',
      () async {
        final file = File(
          '${Directory.systemTemp.path}/volward-session-malformed.json',
        );
        addTearDown(() async {
          if (await file.exists()) await file.delete();
        });
    ...
    test(
      'a missing or truncated session file still leaves the app usable',
      () async {
        final file = File(
          '${Directory.systemTemp.path}/volward-session-truncated.json',
        );
        addTearDown(() async {
          if (await file.exists()) await file.delete();
        });
  ...
  group('resolveStartupRoot fallback', () {
    test(
      'falls back from a stale root to home, then to folder picker',
      () async {
        final session = VolwardSession.test()
    ...
    test(
      'a saved root with a trailing slash normalizes to the launch root',
      () async {
